...
- If Applied: dataset provenance, train/val/test split, baseline comparison, calibrated metrics, a model card
- `submission/video/demo.mp4`: 2:30–4:00

## Strategy notes (starting hypotheses, which the research may overturn)
- TECH is a rare chance to show real ML engineering. The cloud VM is CPU-only (4 vCPU / 16 GB), which comfortably trains a ~5–20M-parameter model on a curated corpus in a few hours. Budget the run, checkpoint it, and log the curves.
- If Applied, prefer Finance: UnivaBio and DSH already cover health.
...
| [`gibc-v2`](https://github.com/rishikrrontala-bot/gibc-v2) | Global Innovation Build Challenge V2 **← this repo** | Thu Oct 1, 2026 · 11:45 AM EDT | TECH (tiny LLM trained from scratch) or Applied-Finance. |
